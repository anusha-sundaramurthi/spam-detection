...
def seed_if_empty():
    ...
    for data in SEEDS:
        ...
        explanation={"spam":{"source_probability":ai.get("spam_probability"),"calculation":"spam_probability / 10","final_score":combined.get("spam_score"),"reasons":risk_reasons,"explanation":"Spam Score is the local model's spam probability converted from 0-100 to 0-10."},"risk":{"starting_score":0,"points_added":combined.get("risk_score"),"final_score":combined.get("risk_score"),"reasons":risk_reasons,"explanation":"Risk begins at 0; the model adds points only for identified spam-risk evidence."},"trust":{"starting_score":10,"points_awarded":combined.get("trust_score"),"points_reduced":round(10-(combined.get("trust_score") or 0),1) if ai["status"]=="complete" else None,"final_score":combined.get("trust_score"),"awarded_reasons":[factor["reason"] for factor in ai["trust_factors"] if factor.get("points",0)>0],"reduction_reasons":risk_reasons or [ai.get("summary")],"explanation":"Trust is shown out of 10; points not awarded are displayed as the reduction from the maximum."}}
