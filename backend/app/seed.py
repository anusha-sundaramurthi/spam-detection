...
def seed_if_empty():
    ...
    for data in SEEDS:
        ...
        doc=data.model_dump()|{"created_at":now,"assessed_at":now,"vendor_id":"vendor@example.com","status":"pending","assessment_version":"ai-only-v1","assessment_status":"complete" if ai["status"]=="complete" else "ai_unavailable","email_normalized":data.email.lower(),"phone_normalized":re.sub(r"\D","",data.phone),"admin_feedback":[],"rule_assessment":rules,"risk_factors":ai["risk_factors"],"trust_factors":ai["trust_factors"],"mandatory_services":MANDATORY_SCORING_SERVICES,"ai_assessment":ai,"combined_assessment":combined,"intelligence":intelligence,**combined}
