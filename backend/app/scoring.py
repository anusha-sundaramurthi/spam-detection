...
def analyze_vendor(data, prior_submissions: list[dict]) -> dict:
    ...
    # FIX: numerator and denominator must come from the same text population.
    # This previously counted uppercase letters in `data.description` only,
    # but divided by the letter count of `combined_text` (title + description +
    # address + package + price + offer combined) — two different populations
    # produce a ratio that doesn't actually represent either field's caps usage.
    description_letters = [c for c in data.description if c.isalpha()]
    caps_ratio = sum(c.isupper() for c in data.description) / max(1, len(description_letters))
    emojis = len(EMOJI_RE.findall(combined_text))
    noisy = (len(description_letters) >= 20 and caps_ratio > .35) or emojis > 5
    ...
    # FIX: data.phone can be falsy/None (see the `if data.phone else` guard used
    # for phone_check just below) — re.sub(pattern, "", None) raises TypeError.
    # Guard with `or ""` so an absent phone is treated as incomplete, not a crash.
    contact_complete = bool(data.email) and len(re.sub(r"\D", "", data.phone or "")) >= 7
