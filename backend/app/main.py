...
from .schemas import AdminDetail, AdminSummary, LoginInput, VendorInput, VendorReceipt, VendorSubmission
...
def ensure_current_assessment(row: dict) -> dict:
    ...
    if row.get("risk_factors") and row.get("trust_factors") and row.get("mandatory_services"):
        return row
    ...
    update = {"assessment_status": "complete", "assessed_at": datetime.now(timezone.utc),
        "rule_assessment": rules, "risk_factors": rules["risk_factors"], "trust_factors": rules["trust_factors"],
        "mandatory_services": MANDATORY_SCORING_SERVICES, "ai_assessment": ai, "combined_assessment": combined, **combined}
...
def admin_submissions(user=Depends(admin_only)):
    ...
    return [serialize(ensure_current_assessment(x)) for x in submissions.find().sort("created_at", -1)]
...
def admin_detail(submission_id: str, user=Depends(admin_only)):
    ...
    return serialize(ensure_current_assessment(row))
