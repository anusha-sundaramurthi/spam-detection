...
export default function Detail(){
  ...
  return <><Link className="back" to="/admin"><ArrowLeft size={17}/>Back to queue</Link>
    ...
    <section className="detail-grid"><div className="panel profile"><h2>Service and package</h2><dl><dt>Package</dt><dd>{item.package_name}</dd><dt>Price</dt><dd>{item.price_or_range}</dd><dt>Timeline</dt><dd>{item.delivery_timeline}</dd><dt>Website</dt><dd>{item.website||'Not supplied'}</dd></dl></div><div className="panel ai-card"><p className="eyebrow">LOCAL AI SPAM REVIEW</p><h2>{item.ai_assessment.model}</h2>{item.ai_assessment.status==='complete'?<><p><b>Spam probability: {item.ai_assessment.spam_probability}%</b></p><p>{item.ai_assessment.summary}</p></>:<p><ShieldAlert size={16}/> {item.ai_assessment.spam_indicators[0]}</p>}</div></section>
