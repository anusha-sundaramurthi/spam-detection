...
MongoDB is normalized into exactly three application collections: `submissions` contains only raw vendor form values, `vendor_id`, `created_at`, and `updated_at`; `assessments` contains workflow status, approvals, text/AI scores, explanations, and reviewer history; and `upload_records` contains protected file metadata plus persisted per-image relevance, visual-spam, integrity, and duplicate results. `submission_id` links assessments and uploads back to the raw submission. Every document has `created_at` and `updated_at`. API reads join these collections without triggering new inference.
...
For every supplied image, the admin detail page displays the model-generated image description (`detected_content`), relevance classification and reason, visual-spam classification and reason, duplicate status, confidence percentage, and model name. These results are persisted in `upload_records`; opening the admin page does not rerun vision inference.
